...
impl DeriveKey for super::Rsa2kPkcs
{
        ...
        -> Result<reply::DeriveKey, Error>
    {
        ...
        let priv_key_der = keystore.load_key(key::Secrecy::Secret, Some(key::Kind::Rsa2k), base_key_id)
...
impl DeserializeKey for super::Rsa2kPkcs
{
        ...
        -> Result<reply::DeserializeKey, Error>
    {
        ...
        let private_key_id = keystore.store_key(
            request.attributes.persistence,
            key::Secrecy::Secret, key::Kind::Rsa2k,
...
impl Sign for super::Rsa2kPkcs
{
        ...
        -> Result<reply::Sign, Error>
    {
        ...
        let native_signature = priv_key
            .sign(PaddingScheme::new_pkcs1v15_sign(Some(Hash::SHA2_256)), &request.message)
            .unwrap();
...
impl Verify for super::Rsa2kPkcs
{
        ...
        -> Result<reply::Verify, Error>
    {
        ...
        let verification_ok = pub_key
            .verify(PaddingScheme::new_pkcs1v15_sign(Some(Hash::SHA2_256)), &request.message, &request.signature)
            .is_ok();

        Ok(reply::Verify { valid: verification_ok })
    }
}
