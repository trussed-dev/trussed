...
#[inline(never)]
fn load_secret_key_cm4(keystore: &mut impl Keystore, key_id: &UniqueId)
    -> Result<p256_cortex_m4::SecretKey, Error>
{
    ...
    let secret_scalar = unsafe { p256_cortex_m4::SecretScalar::unsafe_from_bytes(&secret_scalar) };
    Ok(p256_cortex_m4::SecretKey::from_scalar(&secret_scalar))
}

#[inline(never)]
fn load_public_key_cm4(keystore: &mut impl Keystore, key_id: &UniqueId)
...
impl Agree for super::P256
{
        ...
        -> Result<reply::Agree, Error>
    {
        ...
        let secret_key = load_secret_key_cm4(keystore, &private_id)?;
        let public_key = load_public_key_cm4(keystore, &public_id)?;
...
impl DeriveKey for super::P256
{
        ...
        -> Result<reply::DeriveKey, Error>
    {
        ...
        let secret_key = load_secret_key_cm4(keystore, &base_id)?;
        ...
        -> Result<reply::DeserializeKey, Error>
    {
        ...
        let public_key = match request.format {
            KeySerialization::Cose => {
                ...
                p256_cortex_m4::PublicKey::from_xy_bytes(&serialized_key)
                    .map_err(|_| Error::InvalidSerializedKey)?
            }
...
impl GenerateKey for super::P256
{
        ...
        -> Result<reply::GenerateKey, Error>
    {
        ...
        let key_id = keystore.store_key(
            ...
            key::Secrecy::Secret,
            key::Info::from(key::Kind::P256).with_local_flag(),
            unsafe { keypair.secret.secret_scalar().unsafe_as_bytes() },
        )?;
...
impl SerializeKey for super::P256
{
        ...
        -> Result<reply::SerializeKey, Error>
    {
        ...
        let public_key = load_public_key_cm4(keystore, &key_id)?;
        ...
        let serialized_key = match request.format {
            ...
            KeySerialization::Sec1 => {
                let mut serialized_key = Message::new();
                serialized_key.extend_from_slice(&public_key.to_compressed_bytes()).map_err(|_| Error::InternalError)?;
                serialized_key
            }
        };
...
impl Sign for super::P256
{
        ...
        -> Result<reply::Sign, Error>
    {
        ...
        let secret_key = load_secret_key_cm4(keystore, &key_id)?;
